...
Apparently even with our super device there is a limit to the knowledge that the we can gain about the length of the object. All we can say is that the length of the object is between 83.4 and 83.5 cm. Even if we use smaller and smaller divisions on our meter stick our knowledge about the length of the object will never be perfect. This type of uncertainty is called the **resolution uncertainty**. A general rule of thumb is that the resolution is half of the smallest increment the device can measure. But it may be much greater than this if other known factors are limiting your uncertainty (perhaps you can’t get your ruler close enough to make an accurate measurement or can’t really tell where the object you are trying to measure begins or ends). It may also be less than this in cases where you are able to interpolate while making the measurement. (For example, if you were to perform measurements using a meter stick with no markings, using appropriate interpolation you should be able to estimate your measurements with better accuracy than 0.5m). Estimating uncertainties requires more than rules of thumb; it also requires judgment and common sense.

The "half of the smallest increment" rule of thumb is conceptually reasonable but does not work well quantitatively with a model of uncertainty based on Gaussian distributions, because resolution uncertainty creates more of a flat probability distribution around the measured value.  Therefore, **when combining resolution uncertainty with other uncertainties that follow a roughly Gaussian model, a better estimate of resolution uncertainty is the smallest increment that can be measured *divided by $$\sqrt{12}$$*.**  If you are interested in where this funky numerical value comes from, it is an approximate treatment based on the fact that a flat probability distribution with a width of $$x$$ has a standard deviation of $$x/\sqrt{12}$$.
...
We could continue this process of collecting more data points to try to reduce the random uncertainty. Here are the same two techniques but with $$n=100$$ data points:
![experiment with 100 data points](images/exper-n100.jpg)
By taking more data points, even with the experimental technique that had a larger variability, we were still able to end up with an accurate estimate of the mean, with a smaller estimated uncertainty from the SEM. Taking more data points results in reducing the random effects that pop up in our experiments, and makes our random uncertainty smaller.
