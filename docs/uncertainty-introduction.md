...
The "half of the smallest increment" rule of thumb is conceptually reasonable but does not work well quantitatively with a model of uncertainty based on Gaussian distributions, because resolution uncertainty does not follow a Gaussian distribution.  Therefore, **when combining resolution uncertainty with other uncertainties that do follow a roughly Gaussian model, a better estimate of resolution uncertainty is the smallest increment that can be measured divided by $$\sqrt{12}$$.**  If you are interested in where this funky numerical value comes from, it is an approximate treatment based on the fact that a flat probability distribution with a width of $$x$$ has a standard deviation of $$x/\sqrt{12}$$.
